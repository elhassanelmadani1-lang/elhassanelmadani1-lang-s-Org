...
export default function App() {
  ...
  const copy = spirulinaLandingCopy[lang];
  const tCopy = toothpasteLandingCopy[lang];
  const cCopy = coffeeLandingCopy[lang];
  const cCopy3in1 = coffee3in1LandingCopy[lang];
  const sCopy = shampooLandingCopy[lang];
  const rTrans = reviewTranslations[lang];
  ...
  const handleNavigateView = (view: typeof activeView) => {
    ...
    if (activeView !== 'store' && activeView !== 'shop' && activeView !== 'bestsellers' && activeView !== 'checkout' && activeView !== 'gano-oil' && activeView !== 'limonzhi' && activeView !== 'soap' && activeView !== 'reishilium' && activeView !== 'reishi_gano') {
      try {
  ...
  const getProductDetailsForCart = (productId: string) => {
    ...
    if (productId === 'toothpaste') {
      return {
        name: productsData.toothpaste[lang].name,
    ...
    if (productId === 'coffee') {
      return {
        name: productsData.coffee[lang].name,
    ...
    if (productId === 'reishilium' || productId === 'reishilium_small') {
      ...
      return {
        name: `${productsData.reishilium[lang].name}${suffix}`,
        price: 401,
    ...
    if (productId === 'reishi_gano' || productId === 'rg_90' || productId === 'reishi_gano_small') {
      ...
      return {
        name: `${productsData.reishi_gano[lang].name}${suffix}`,
        price: 706,
    ...
    if (productId === 'spirulina_500') {
      ...
      return {
        name: `${productsData.spirulina[lang].name}${suffix}`,
        price: 1299,
    ...
    const normalizeId = (id: string) => {
      ...
      if (id === 'morinzhi-juice') return 'morinzhiJuice';
    ...
    if (prod) {
      return {
        name: prod[lang]?.name || prod['en']?.name || normalized,
        price: prod.price,
        image: prod.image || '',
      };
    }
    ...
    if (matchedPack) {
      return {
        name: matchedPack.name[lang] || matchedPack.name['en'],
  ...
  // Clean up any previously stored customer details so forms start empty
  useEffect(() => {
  ...
  const handleLandingOrderSubmit = (e: FormEvent) => {
    ...
    let addonsText = '';
    if (addToothpaste) addonsText += `• ${lang === 'ar' ? 'معجون أسنان جانوزي بلس (+85 درهم)' : 'Dentifrice DXN Ganozhi Plus (+85 DH)'}\n`;
    if (addCoffee) addonsText += `• ${lang === 'ar' ? 'قهوة لينجزي السوداء (+130 درهم)' : 'Café noir Gourmet Lingzhi (+130 DH)'}\n`;

    const freeShipping = spirulinaQty > 1;

    const message = 
`👑 ${lang === 'ar' ? 'طلب شراء فاخر - سميرة ناتورال' : 'COMMANDE DE BIEN-ÊTRE - SAMIRA NATURALE'} 👑
...
---------------------------------------
📦 ${isRtl ? 'الكمية المختارة' : 'Quantité Sélectionnée'}:
• ${spirulinaQty} × ${spirulinaVariant === '120' ? (isRtl ? 'سبيرولينا عضوية (120 حبة)' : 'Spiruline Biologique (120 comprimés)') : (isRtl ? 'سبيرولينا عضوية (500 حبة)' : 'Spiruline Biologique (500 comprimés)')} [${getBasePrice()} ${t.currency}]
...
• ${isRtl ? 'المتابعة الطبية' : 'Suivi'}: ${isRtl ? 'استشارة وصية هاتفية هدية مع لالة سميرة' : 'Consultation exclusive incluse'}
${spirulinaQty >= 3 ? (isRtl ? '• ملعقة خشبية لتحديد الجرعة هدية مجانية' : '• Cuillère doseuse bambou offerte') : ''}
---------------------------------------
💵 ${isRtl ? 'المجموع النهائي للدفع' : 'TOTAL À PAYER'}: ${totalPrice} ${t.currency}

🚀 ${isRtl ? 'أهلاً لالة سميرة، أود تأكيد طلبي لمنتجات السبيرولينا العضوية المذكورة أعلاه. يرجى شحن الطرد في أقرب وقت!' : 'Bonjour Samira, je souhaite confirmer ma commande ci-dessus de Spiruline Biologique DXN. Merci de me contacter pour l\'expédition !'}`;
  ...
  const handleToothpasteOrderSubmit = (e: FormEvent) => {
    ...
    let addonsText = '';
    if (addToothpasteSpirulina) addonsText += `• ${lang === 'ar' ? 'مسحوق السبيرولينا العضوي (+260 درهم)' : 'Poudre de Spiruline Biologique (+260 DH)'}\n`;
    if (addToothpasteCoffee) addonsText += `• ${lang === 'ar' ? 'قهوة لينجزي السوداء (+130 درهم)' : 'Café noir Gourmet Lingzhi (+130 DH)'}\n`;

    const freeShipping = toothpasteQty > 1;

    const message = 
`👑 ${lang === 'ar' ? 'طلب شراء معجون أسنان فاخر - سميرة ناتورال' : 'COMMANDE DE DENTIFRICE PREMIUM - SAMIRA NATURALE'} 👑
...
---------------------------------------
📦 ${isRtl ? 'الكمية المطلوبة لمعجون الأسنان' : 'Quantité Dentifrice Sélectionnée'}:
• ${toothpasteQty} × ${isRtl ? 'معجون أسنان جانوزي الطبيعي' : 'Dentifrice Ganozhi Plus DXN'} [${85 * toothpasteQty} ${t.currency}]
...
${toothpasteQty >= 3 ? (isRtl ? '• فرشاة أسنان خشبية فاخرة وصديقة للبيئة هدية مجانية' : '• Brosse à dents bambou offerte') : ''}
---------------------------------------
💵 ${isRtl ? 'المجموع النهائي للدفع' : 'TOTAL À PAYER'}: ${totalPrice + (toothpasteQty === 1 ? 20 : 0)} ${t.currency}

🚀 ${isRtl ? 'أهلاً لالة سميرة، أود تأكيد طلبي لمنتج معجون أسنان جانوزي بلس بالجانوديرما المذكور أعلاه. يرجى شحن الطرد في أقرب وقت!' : 'Bonjour Samira, je souhaite confirmer ma commande ci-dessus de Dentifrice Ganozhi Plus. Merci de me contacter pour l\'expédition !'}`;
  ...
  const handleCoffeeOrderSubmit = (e: FormEvent) => {
    ...
    let addonsText = '';
    if (addCoffeeSpirulina) addonsText += `• ${lang === 'ar' ? 'مسحوق السبيرولينا العضوي (+260 درهم)' : 'Poudre de Spiruline Biologique (+260 DH)'}\n`;
    if (addCoffeeToothpaste) addonsText += `• ${lang === 'ar' ? 'معجون أسنان جانوزي بلس (+85 درهم)' : 'Dentifrice Prestige Ganozhi (+85 DH)'}\n`;

    const freeShipping = coffeeQty > 1;

    const message = 
`👑 ${lang === 'ar' ? 'طلب شراء قهوة لينجزي الفاخرة - سميرة ناتورال' : 'COMMANDE DE CAFÉ DE PRESTIGE LINGZHI - SAMIRA NATURALE'} 👑
...
---------------------------------------
📦 ${isRtl ? 'الكمية المطلوبة لقهوة لينجزي' : 'Quantité Café Sélectionnée'}:
• ${coffeeQty} × ${isRtl ? 'قهوة لينجزي السوداء 2 في 1' : 'Café Noir Gourmet DXN'} [${130 * coffeeQty} ${t.currency}]
...
• ${isRtl ? 'التوصيل والضمان' : 'Livraison'}: ${freeShipping ? (isRtl ? 'شحن سريع ومجاني بالكامل' : 'Livraison Express OFFERTE') : (isRtl ? 'شحن محلي مدفوع' : 'Frais de port normaux')}
• ${isRtl ? 'المتابعة والتوجيه' : 'Suivi'}: ${isRtl ? 'استشارة ومتابعة هاتفية مخصصة من لالة سميرة' : 'Consultation exclusive incluse'}
${coffeeQty >= 2 ? (isRtl ? '• كوب سفر فاخر أو ملعقة خشبية هدية مجانية' : '• Tasse ou spatule offerte') : ''}
---------------------------------------
💵 ${isRtl ? 'المجموع النهائي للدفع' : 'TOTAL À PAYER'}: ${totalPrice + (coffeeQty === 1 ? 20 : 0)} ${t.currency}

🚀 ${isRtl ? 'أهلاً لالة سميرة، أود تأكيد طلبي لمنتج قهوة لينجزي السوداء 2 في 1 المذكورة أعلاه. يرجى شحن الطرد في أقرب وقت!' : 'Bonjour Samira, je souhaite confirmer ma commande ci-dessus de Café Noir DXN Lingzhi 2 en 1. Merci de me contacter pour l\'expédition !'}`;
...
🚀 ${isRtl ? 'أهلاً لالة سميرة، أود تأكيد طلبي لمنتج قهوة لينجزي 3 في 1 المذكورة أعلاه. يرجى شحن الطرد في أقرب وقت!' : 'Bonjour Samira, je souhaite confirmer ma commande ci-dessus de Café Crème DXN 3 en 1. Merci de me contacter pour l\'expédition !'}`;
  ...
  const handleShampooOrderSubmit = (e: FormEvent) => {
    ...
    let addonsText = '';
    if (addShampooSpirulina) addonsText += `• ${lang === 'ar' ? 'مسحوق السبيرولينا العضوي (+260 درهم)' : 'Poudre de Spiruline Biologique (+260 DH)'}\n`;
    if (addShampooToothpaste) addonsText += `• ${lang === 'ar' ? 'معجون أسنان جانوزي بلس (+85 درهم)' : 'Dentifrice Prestige Ganozhi (+85 DH)'}\n`;
    if (addShampooCoffee3in1) addonsText += `• ${lang === 'ar' ? 'قهوة لينجزي 3 في 1 (+140 درهم)' : 'Café Prestige Lingzhi 3in1 (+140 DH)'}\n`;

    const freeShipping = shampooQty > 1;

    const message = 
`👑 ${lang === 'ar' ? 'طلب شراء شامبو جانوزي الفاخر - سميرة ناتورال' : 'COMMANDE DE SHAMPOING DE PRESTIGE GANOZHI - SAMIRA NATURALE'} 👑
...
---------------------------------------
📦 ${isRtl ? 'الكمية المطلوبة لشامبو جانوزي' : 'Quantité de Shampoing'}:
• ${shampooQty} × ${isRtl ? 'شامبو جانوزي دي إكس إن الطبيعي' : 'Shampoing DXN Ganozhi'} [${120 * shampooQty} ${t.currency}]
...
• ${isRtl ? 'التوصيل والضمان' : 'Livraison'}: ${freeShipping ? (isRtl ? 'شحن سريع ومجاني بالكامل' : 'Livraison Express OFFERTE') : (isRtl ? 'شحن محلي مدفوع' : 'Frais de port normaux')}
• ${isRtl ? 'المتابعة والتوجيه' : 'Suivi'}: ${isRtl ? 'استشارة ومتابعة هاتفية مخصصة من لالة سميرة' : 'Consultation exclusive incluse'}
${shampooQty >= 2 ? (isRtl ? '• مشط تدليك خشبي فاخر مضاد للشحنات هدية مجانية' : '• Peigne luxueux en bois de santal offert') : ''}
---------------------------------------
💵 ${isRtl ? 'المجموع النهائي للدفع' : 'TOTAL À PAYER'}: ${totalPrice + (shampooQty === 1 ? 20 : 0)} ${t.currency}

🚀 ${isRtl ? 'أهلاً لالة سميرة، أود تأكيد طلبي لمنتج شامبو جانوزي دي إكس إن الطبيعي المذكور أعلاه. يرجى شحن الطرد في أقرب وقت!' : 'Bonjour Samira, je souhaite confirmer ma commande ci-dessus de Shampoing DXN Ganozhi. Merci de me contacter pour l\'expédition !'}`;
  ...
  const handleGeneralCartSubmit = (e: FormEvent) => {
    ...
    Object.entries(cart).forEach(([pId, qty]) => {
      ...
      grandTotal += price;
      itemsText += `• ${resolved.name} (${t.quantity}: ${qty}) - ${price} DH\n`;
    });

    const message = `👑 ${t.whatsappMessageHeader} 👑

👤 ${isRtl ? 'الاسم الكامل للزبون' : 'Nom du Client'}: ${customerName}
📍 ${t.addressLabel}: ${customerAddress}
...
🎁 ${isRtl ? 'الشحن والمتابعة' : 'Livraison & Suivi'}:
• ${isRtl ? 'الدفع عند الاستلام مع شحن سريع ومجاني بالكامل' : 'Paiement à la livraison + Livraison Express OFFERTE'}
• ${isRtl ? 'متابعة هاتفية هدية مستمرة مع لالة سميرة' : 'Conseil et suivi direct personnalisé de Samira inclus'}
---------------------------------------
💵 ${t.totalLabel}: ${grandTotal} ${t.currency}

🚀 ${isRtl ? 'أهلاً لالة سميرة، أود تأكيد طلبي لهذه المنتجات الطبيعية من متجرك الرائع. يرجى تجهيز الطرد وشحنه لي في أقرب وقت!' : 'Bonjour Samira, je souhaite confirmer ma commande ci-dessus de vos merveilleux produits DXN. Merci de me contacter pour l\'expédition !'}`;
    ...
    >
                ...
                <div className="border-t border-slate-100 pt-5 mt-auto bg-white space-y-4">
                  ...
                  <form onSubmit={handleGeneralCartSubmit} className="space-y-3 pt-3 border-t border-slate-100">
                    ...
                    <button
                      type="submit"
                      disabled={isOrdering}
                      className="w-full py-3 bg-[#1C352D] hover:bg-[#25443B] text-white rounded-xl text-center font-extrabold text-xs uppercase tracking-wider shadow-md transition-all active:scale-95 disabled:opacity-50 cursor-pointer"
                    >
              ...
              <div className="flex-1 py-8 overflow-y-auto space-y-5">
                ...
                <div className="pt-8 border-t border-black/[0.05] dark:border-white/[0.05] space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center rounded-full p-0.5 bg-black/[0.03] dark:bg-white/[0.03] border border-black/[0.04] dark:border-white/[0.06]">
                      {(['ar', 'fr', 'en'] as LanguageCode[]).map((langOption) => (
                        <button
                          key={langOption}
                          onClick={() => setLang(langOption)}
                          className={`px-3 py-1 text-[11px] font-extrabold rounded-full transition-all cursor-pointer ${
                            lang === langOption
                              ? 'bg-[#1F4D3A] dark:bg-[#C9A227] text-white dark:text-slate-950 font-black'
                              : 'text-slate-400'
                          }`}
                        >
      ...
      <motion.header
        ...
        className={`sticky top-0 z-45 w-full select-none transition-all duration-400 ease-in-out ${
          isScrolled
            ? 'h-12 md:h-[68px] bg-white/80 dark:bg-slate-950/80 backdrop-blur-md shadow-[0_12px_40px_rgba(0,0,0,0.08)] border-b border-black/[0.04] dark:border-white/[0.04]'
            : 'h-12 md:h-[80px] bg-white dark:bg-slate-950 border-b border-black/[0.06] dark:border-white/[0.06]'
        }`}
      >
        <div className="max-w-7xl mx-auto h-full px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          ...
          <div className="hidden md:grid grid-cols-12 items-center w-full h-full">
            ...
            <div className="col-span-4 flex items-center justify-end gap-3.5">
              ...
              {/* Premium Language Switcher (Segmented Control Rounded Pill) */}
              <div className="flex items-center rounded-full p-1 bg-[#FAF8F3] dark:bg-slate-900 border border-black/[0.04] dark:border-white/[0.06] shadow-xs relative">
                {(['ar', 'fr', 'en'] as LanguageCode[]).map((langOption) => {
                  ...
                  return (
                    <button
                      key={langOption}
                      onClick={() => setLang(langOption)}
                      className="relative px-3.5 py-1 text-xs font-semibold rounded-full transition-all duration-300 cursor-pointer"
                    >
                      {isActive && (
                        <motion.div
                          layoutId="desktopActiveLang"
                          className="absolute inset-0 bg-[#1F4D3A] dark:bg-[#C9A227] rounded-full animate-none"
                          transition={{ type: "spring", stiffness: 380, damping: 28 }}
                        />
                      )}
                      <span className={`relative z-10 block transition-transform duration-200 hover:scale-105 ${
                        isActive 
                          ? 'text-white dark:text-slate-950 font-bold' 
                          : 'text-[#1B1B1B]/70 dark:text-slate-300/70 hover:text-[#1B1B1B] dark:hover:text-white'
                      }`}>
                        {langOption === 'ar' ? 'العربية' : langOption === 'fr' ? 'Français' : 'English'}
                      </span>
                    </button>
                  );
                })}
          ...
          <div className="flex md:hidden items-center justify-between w-full h-full relative">
            ...
            <div className="flex items-center gap-1.5 z-10">
              {/* Language toggle to balance visual weight */}
              <button
                onClick={() => setLang(lang === 'ar' ? 'fr' : 'ar')}
                className="text-[10px] font-extrabold text-[#1F4D3A] dark:text-white px-2 py-1 rounded-md bg-[#FAF8F3]/60 dark:bg-slate-900/50 cursor-pointer border border-black/[0.03] dark:border-white/[0.04]"
              >
                {lang === 'ar' ? 'FR' : 'عربي'}
              </button>
      ...
      <section id="benefits-section" className="py-16 sm:py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {benefitsData[lang].map((item, index) => (
              <div
                key={index}
      ...
      <section id="faq-section" className="py-16 sm:py-24 bg-white border-t border-[#E5D5C0]/40">
        ...
        >
          ...
          <section id="toothpaste_benefits" className="py-16 sm:py-24 bg-[#FAF6F0]/25 border-b border-[#E5D5C0]/40">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {toothpasteBenefits[lang].map((item, index) => (
                  <div
                    key={index}
          ...
          <section id="toothpaste_how_to_use" className="py-16 sm:py-24 bg-[#FAF6F0]/25 border-b border-[#E5D5C0]/40">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="grid sm:grid-cols-3 gap-8 max-w-5xl mx-auto relative text-center">
                {toothpasteSteps[lang].map((step, idx) => (
          ...
          <section id="toothpaste_faq_section" className="py-16 sm:py-24 bg-white border-t border-[#E5D5C0]/40">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <FAQAccordion lang={lang} customItems={toothpasteFAQs[lang]} />
            </div>
          </section>
        </motion.div>
      ) : activeView === 'coffee' ? (
        ...
        >
          ...
          <section id="coffee-benefits" className="py-16 sm:py-24 bg-white border-b border-[#FAF6F0]">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
                {coffeeBenefits[lang].map((item, index) => {
                  return (
                    <div
                      key={index}
          ...
          <section id="coffee-ritual" className="py-16 sm:py-24 bg-[#FAF6F0] border-b border-[#E5D5C0]/40">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
                {coffeeSteps[lang].map((step, index) => {
                  return (
                    <div
                      key={index}
          ...
          <section id="coffee_faq_section" className="py-16 sm:py-24 bg-[#FAF6F0] text-slate-800 border-t border-[#E5D5C0]/40">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <FAQAccordion lang={lang} customItems={coffeeFAQs[lang]} />
        ...
        >
          ...
          <section id="coffee3in1_benefits" className="py-16 sm:py-24 bg-[#FCFAF5] border-b border-[#FAF6F0]">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
                {coffee3in1Benefits[lang].map((benefit, index) => {
                  return (
                    <div
                      key={index}
          ...
          <section id="coffee3in1_how_to_use" className="py-16 sm:py-24 bg-[#FCFAF5] border-b border-[#FAF6F0]">
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="grid sm:grid-cols-4 gap-8 relative">
                {coffee3in1Steps[lang].map((step, idx) => {
                  return (
          ...
          <section id="coffee3in1_faq_section" className="py-16 sm:py-24 bg-[#FAF6F0] text-slate-800 border-t border-[#E5D5C0]/40">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <FAQAccordion lang={lang} customItems={coffee3in1FAQs[lang]} />
        ...
        <GanoMassageOilPage lang={lang} isRtl={isRtl} t={t} WHATSAPP_PHONE={WHATSAPP_PHONE} />
      ) : activeView === 'limonzhi' ? (
        <LimonzhiPage lang={lang} isRtl={isRtl} t={t} WHATSAPP_PHONE={WHATSAPP_PHONE} />
      ) : activeView === 'soap' ? (
        <GanozhiSoapPage lang={lang} isRtl={isRtl} t={t} WHATSAPP_PHONE={WHATSAPP_PHONE} />
      ) : activeView === 'reishilium' ? (
        <ReishiliumPowderPage lang={lang} isRtl={isRtl} t={t} WHATSAPP_PHONE={WHATSAPP_PHONE} />
      ) : activeView === 'shop' ? (
        <motion.div
          key="shop-view"
          initial={{ opacity: 0 }}
        ...
        >
          ...
          <section id="shampoo_benefits" className="py-16 sm:py-24 bg-[#FCFAF5] border-b border-[#FAF6F0]">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto font-sans">
                {shampooBenefits[lang].map((benefit, index) => {
          ...
          <section id="shampoo_how_to_use" className="py-16 sm:py-24 bg-[#FCFAF5] border-b border-[#FAF6F0]">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8 max-w-5xl mx-auto relative select-none font-sans">
                {shampooSteps[lang].map((step, idx) => {
                  return (
          ...
          <section id="shampoo_faq_section" className="py-16 sm:py-24 bg-[#FAF6F0] text-slate-800 border-t border-[#E5D5C0]/40">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <FAQAccordion lang={lang} customItems={shampooFAQs[lang]} />
      ...
      <div className={`fixed right-6 z-[9999] group transition-all duration-300 ${
        ['spirulina', 'toothpaste', 'coffee', 'coffee3in1', 'shampoo', 'gano-oil', 'limonzhi', 'soap', 'reishilium'].includes(activeView)
      ...
      {['spirulina', 'toothpaste', 'coffee', 'coffee3in1', 'shampoo', 'gano-oil', 'limonzhi', 'soap', 'reishilium'].includes(activeView) && (
        <div className="fixed bottom-0 inset-x-0 bg-white/95 dark:bg-slate-950/95 backdrop-blur-md border-t border-neutral-200/50 dark:border-white/[0.06] p-3 z-[9998] flex md:hidden items-center justify-between gap-3 shadow-[0_-8px_30px_rgba(0,0,0,0.08)] select-none animate-slide-up">
          <div className="flex flex-col">
            <span className="text-[9px] text-slate-400 font-bold uppercase leading-none">
              {lang === 'ar' ? 'السعر الحصري' : 'Prix exclusif'}
            </span>
            <span className="text-base font-black text-[#1C352D] dark:text-white mt-1 leading-none font-mono">
              {activeView === 'spirulina' ? (spirulinaVariant === 'big' ? '540' : '290')
               : activeView === 'toothpaste' ? '120'
               : activeView === 'coffee' ? '280'
               : activeView === 'coffee3in1' ? '299'
               : activeView === 'shampoo' ? '199'
               : activeView === 'gano-oil' ? '199'
               : activeView === 'limonzhi' ? '280'
               : activeView === 'soap' ? '130'
               : '290'}{' '}
              <span className="text-[10px] font-bold text-[#C5A560]">
                {lang === 'ar' ? 'درهم' : 'DH'}
              </span>
            </span>
          </div>
          ...
          <button
            onClick={() => {
              if (activeView === 'spirulina') {
                scrollToCheckout();
              } else if (activeView === 'toothpaste') {
                scrollToToothpasteCheckout();
              } else if (activeView === 'coffee') {
                scrollToCoffeeCheckout();
              } else if (activeView === 'coffee3in1') {
                scrollToCoffee3in1Checkout();
              } else if (activeView === 'shampoo') {
                scrollToShampooCheckout();
              } else {
                // premium products
                const el = document.getElementById('premium-checkout-section') || document.getElementById('premium-checkout-form-container');
                el?.scrollIntoView({ behavior: 'smooth' });
            ...
            className="flex-1 py-3 px-5 bg-[#1C352D] dark:bg-[#C9A227] text-white dark:text-slate-950 font-black text-[11px] uppercase tracking-wider rounded-xl flex items-center justify-center gap-2 shadow-md active:scale-95 transition-all cursor-pointer"
          >
            <ShoppingBag className="w-3.5 h-3.5 shrink-0 text-white dark:text-slate-950" />
            <span>{isRtl ? 'اطلب الآن مجاناً 🚚' : 'Acheter Maintenant'}</span>
          </button>
        </div>
      )}
