...
export default function DXNPremiumProductsPage({
  ...
  const products: ProductDetails[] = [
    ...
    },
    {
      ...
      desc: {
        ar: "السبيرولينا هي الطحالب الخضراء المزرقة القلوية الفائقة، معترف بها عالمياً كأغنى مصدر طبيعي للبروتينات العضوية، الحديد، المعادن النادرة، الكلوروفيل، والفيتامينات لرفع المناعة ومحاربة فقر الدم.",
  ...
  allProducts.forEach(p => {
    ...
    const excludeIds = [
      'coffee', 'coffee3in1', 'spirulina', 'morinzhi', 'morinzhi_juice', 'morinzhi-juice',
      'gano-oil', 'gano_oil', 'cocozhi', 'toothpaste', 'shampoo', 'soap', 'cordyceps',
      'lions-mane', 'lions_mane', 'limonzhi', 'reishilium', 'reishi_gano', 'cordypine',
      ...
      'pack-energy', 'pack-immunity', 'pack-wellness'
    ];
  ...
  const getProductIndexById = (id: string | null | undefined) => {
    ...
    const idx = products.findIndex(p => p.id === id || p.id === normalizedId || p.id.replace(/-/g, '_') === normalizedId);
    return idx >= 0 ? idx : 0;
  };
...
🚀 أهلاً، أود تأكيد طلبي لمنتج ${activeProductTitle}${sizeStr} المذكور أعلاه. يرجى شحن الطرد في أقرب وقت ومراسلتي عبر الواتساب لتأكيد الشحن!`;
  ...
  const text = {
    ...
    fr: {
      ...
      stockAlert: "🚨 Stocks extrêmement limités ! Réservez votre cure dès aujourd'hui",
    ...
    <div className="w-full min-h-screen bg-[#FAF7F2] text-slate-800" style={{ direction: isRtl ? 'rtl' : 'ltr' }}>
      ...
      {productId ? (
        <div className="bg-[#FAF6F0] border-b border-emerald-100 py-3.5 text-center sm:px-6 select-none flex flex-wrap items-center justify-center gap-2.5 text-xs font-bold text-[#1C352D]">
          ...
          >
            {lang === 'ar' ? 'الرجوع للمتجر للتسوق العام' : 'Retour au catalogue général'}
          </button>
        </div>
      ) : (
          ...
          <form onSubmit={handleOrderSubmit} className="space-y-6 relative z-10">
            ...
            <div className="pt-2">
              <button
                ...
                className="w-full inline-flex items-center justify-center gap-3 px-8 py-4.5 bg-emerald-800 hover:bg-[#1E3A34] text-white font-extrabold text-sm sm:text-base rounded-2xl shadow-xl hover:shadow-emerald-800/10 transition-all duration-300 border-2 border-[#C5A560]/30 transform active:scale-98 cursor-pointer"
              >
                {isSubmitting ? (
