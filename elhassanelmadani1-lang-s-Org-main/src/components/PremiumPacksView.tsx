...
export default function PremiumPacksView({
  ...
  const filteredPacks = useMemo(() => {
    return wellnessPacks.filter(pack => {
      ...
      if (activeFilter !== 'all') {
        ...
        if (activeFilter === 'cosmetics' && pack.id !== 'luxury-cosmetics-pack' && pack.id !== 'lifting-antiaging-pack' && pack.id !== 'supreme-lifting-elixir-pack' && pack.id !== 'sun-protection-tanning-pack') return false;
      }
      // 2. Filter by search query
      if (searchQuery.trim() !== '') {
        const query = searchQuery.toLowerCase();
        const nameMatch = pack.name[lang].toLowerCase().includes(query);
        const descMatch = pack.desc[lang].toLowerCase().includes(query);
        const benefitsMatch = pack.benefits[lang].some(b => b.toLowerCase().includes(query));
  ...
  const handleDirectCodSubmit = (e: FormEvent) => {
    ...
    const productsListText = currentPack.products.map(p => `• ${p.name[lang]}`).join('\n');
    ...
    const textMessage = lang === 'ar' 
      ...
      : `*NOUVELLE COMMANDE - PACK PREMIUM* 🌿✨\n\n` +
        `*Nom du Pack :* ${currentPack.name[lang]}\n` +
              ...
              <div className="lg:col-span-7 space-y-10">
                    ...
                    {currentPack.products.map((prod) => (
                      ...
                      >
                        ...
                        <div className="space-y-1 text-center sm:text-right flex-1" style={{ textAlign: isRtl ? 'right' : 'left' }}>
                          <h3 className="text-base font-extrabold text-[#1C352D]">{prod.name[lang]}</h3>
                ...
                <div className="bg-white rounded-3xl border border-[#EADFC9]/45 p-6 sm:p-8 space-y-6">
                  ...
                  <div className="grid gap-4">
                    {currentPack.benefits[lang].map((benefit, idx) => (
                      <div 
                        key={idx} 
                ...
                <div className="bg-white rounded-3xl border border-[#EADFC9]/45 p-6 sm:p-8 space-y-6">
                  ...
                  <div className="grid gap-3.5">
                    {currentPack.whoIsItFor[lang].map((target, idx) => (
                  ...
                  <div ref={orderFormRef} className="pt-4 border-t border-slate-100 space-y-4">
                    ...
                    {formSubmitted ? (
                        ...
                        <a 
                          href={`https://wa.me/212648834419?text=${encodeURIComponent(
                            lang === 'ar' 
                              ...
                              : `Commande du Pack: ${currentPack.name[lang]}\nNom: ${checkoutName}\nTéléphone: ${checkoutPhone}`
                          )}`}
                          target="_blank"
                          rel="noreferrer"
                      ...
                      <form onSubmit={handleDirectCodSubmit} className="space-y-4">
                        ...
                        <button
                          type="submit"
                          className="w-full py-4 px-4 bg-[#1C352D] hover:bg-[#25443B] text-white font-extrabold text-xs uppercase tracking-wider rounded-xl transition-all shadow-md active:scale-97 cursor-pointer border border-[#D4AF37]/20"
                        >
                          {t.submitOrder}
                        </button>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-16" style={{ direction: isRtl ? 'rtl' : 'ltr' }}>
              <div className="bg-white rounded-3xl border border-[#EADFC9]/45 p-6 sm:p-8 space-y-6">
                ...
                <div className="grid sm:grid-cols-3 gap-6 pt-2">
                  {currentPack.usage[lang].map((instruction, idx) => (
                    <div 
                      key={idx} 
