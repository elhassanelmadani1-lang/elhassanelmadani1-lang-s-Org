...
export default function LimonzhiPage({ lang, isRtl, t, WHATSAPP_PHONE }: LimonzhiPageProps) {
  const copy = limonzhiLandingCopy[lang];
  const benefits = limonzhiBenefits[lang];
  const features = limonzhiFeatures[lang];
  const steps = limonzhiSteps[lang];
  const faqs = limonzhiFAQs[lang];
  const packs = limonzhiPacks[lang];
  const whyChooseItems = whyChooseLimonzhiList[lang];
      ...
      <section className="py-20 sm:py-28 bg-white border-b border-stone-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {limonzhiReviews[lang].map((review) => (
              <div 
                key={review.id}
              ...
              {orderCompleted ? (
                ...
                <form onSubmit={handleOrderSubmit} className="space-y-5">
                  ...
                  <button
                    type="submit"
                    disabled={isOrdering}
                    className="w-full py-4 px-6 bg-gradient-to-r from-yellow-350 to-yellow-400 hover:from-yellow-450 hover:to-yellow-500 text-slate-950 font-extrabold rounded-xl text-xs sm:text-sm uppercase tracking-wider shadow-lg transition-all hover:scale-101 flex items-center justify-center gap-2 cursor-pointer"
                  >
                    {isOrdering ? (
