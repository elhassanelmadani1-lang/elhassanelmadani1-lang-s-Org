...
export default function ReishiliumPowderPage({ lang, isRtl, t, WHATSAPP_PHONE }: ReishiliumPowderPageProps) {
  const copy = reishiliumLandingCopy[lang];
  const benefits = reishiliumBenefits[lang];
  const features = reishiliumFeatures[lang];
  const steps = reishiliumSteps[lang];
  const faqs = reishiliumFAQs[lang];
  const packs = reishiliumPacks[lang];
  const whyChooseItems = whyChooseReishiliumList[lang];
    ...
    <div id="dxn-reishilium-landing" className={`min-h-screen bg-[#FAF9F6] text-stone-800 ${isRtl ? 'rtl text-right' : 'ltr text-left'}`}>
                  ...
                  <form onSubmit={handleOrderSubmit} className="space-y-5">
                    ...
                    <div className="pt-2">
                      <button
                        type="submit"
                        disabled={isOrdering}
                        className="w-full inline-flex items-center justify-center gap-3 bg-gradient-to-r from-[#D4A64A] via-[#EAE1D1] to-[#D4A64A] text-stone-950 font-black text-xs sm:text-sm uppercase tracking-widest hover:brightness-110 active:scale-98 transition-all py-4 px-6 rounded-xl cursor-pointer shadow-lg shadow-[#D4A64A]/10 disabled:bg-stone-600 disabled:text-stone-400"
                      >
                        {isOrdering ? (
      ...
      <section className="py-20 sm:py-28 bg-[#FAF9F6]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {reishiliumReviews[lang].map((review) => (
              <div
                key={review.id}
