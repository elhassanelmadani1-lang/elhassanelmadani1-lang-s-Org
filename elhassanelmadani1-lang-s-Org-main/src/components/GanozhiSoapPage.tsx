...
export default function GanozhiSoapPage({ lang, isRtl, t, WHATSAPP_PHONE }: GanozhiSoapPageProps) {
  const copy = ganoSoapLandingCopy[lang];
  const benefits = ganoSoapBenefits[lang];
  const features = ganoSoapFeatures[lang];
  const steps = ganoSoapSteps[lang];
  const faqs = ganoSoapFAQs[lang];
  const packs = ganoSoapPacks[lang];
  const whyChooseItems = whyChooseSoapList[lang];
                  ...
                  {!orderCompleted ? (
                    ...
                    >
                      ...
                      <button
                        type="submit"
                        disabled={isOrdering}
                        className="w-full py-4 bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-500 hover:to-amber-600 disabled:opacity-50 text-slate-950 font-black text-xs sm:text-sm rounded-xl tracking-wider uppercase transition-all duration-300 flex items-center justify-center gap-2 hover:translate-y-[-1px] cursor-pointer shadow-lg shadow-amber-500/5"
                      >
