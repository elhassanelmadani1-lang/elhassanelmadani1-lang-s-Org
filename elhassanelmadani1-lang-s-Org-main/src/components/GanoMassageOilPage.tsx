...
export default function GanoMassageOilPage({ lang, isRtl, t, WHATSAPP_PHONE }: GanoMassageOilPageProps) {
  const copy = ganoMassageOilLandingCopy[lang];
  const benefits = ganoMassageOilBenefits[lang];
  const features = ganoMassageOilFeatures[lang];
  const steps = ganoMassageOilSteps[lang];
  const faqs = ganoMassageOilFAQs[lang];
  const packs = ganoMassageOilPacks[lang];
  const whyChooseItems = whyChooseGanoList[lang];
                    ...
                    <form onSubmit={handleOrderSubmit} className="space-y-6 relative z-10">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <input 
                          ...
                          required
                          className="w-full text-xs sm:text-sm px-4 py-3 rounded-xl border border-stone-800 bg-white/5 text-stone-100 placeholder-stone-500 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 font-semibold transition-all font-mono"
                        />
                      </div>
                      ...
                      <button
                        type="submit"
                        disabled={isOrdering}
                        className="w-full py-4 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-stone-950 font-black text-xs sm:text-sm uppercase tracking-wider rounded-xl transition-all shadow-lg hover:scale-102 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
                      >
                        {isOrdering ? (
