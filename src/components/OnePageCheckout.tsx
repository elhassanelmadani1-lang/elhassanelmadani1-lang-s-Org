          ...
          <div className="grid lg:grid-cols-12 gap-8 items-start">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <input
                        ...
                        className={`w-full px-4 py-3.5 rounded-xl bg-[#FAF6F0] text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#1C352D] border dark:bg-slate-700 dark:text-white dark:focus:ring-[#C5A560] text-xs sm:text-sm font-bold font-mono ${
                          phone && !isValidMoroccanPhone(phone)
                            ? 'border-rose-500 focus:ring-rose-500 focus:border-rose-500'
                            : 'border-emerald-100 dark:border-slate-600'
                        }`}
                      />
                    </div>
                    {phone && !isValidMoroccanPhone(phone) && (
            ...
            <div className="lg:col-span-5 space-y-6">
              ...
              <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 sm:p-8 border border-emerald-100/40 dark:border-slate-700 shadow-xl select-none sticky top-6">
                ...
                <button
                  type="button"
                  disabled={isOrdering || !name.trim() || !phone.trim() || !city.trim() || !address.trim() || !isValidMoroccanPhone(phone)}
                  onClick={() => document.getElementById('checkout-form-submit-btn')?.click()}
                  className="w-full mt-8 px-6 py-4 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs sm:text-sm uppercase tracking-wide flex items-center justify-center gap-2.5 shadow-xl shadow-emerald-600/10 transition-all duration-300 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                >
