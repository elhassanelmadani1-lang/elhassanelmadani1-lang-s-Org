...
export default function PremiumCheckoutForm({
  ...
  const t = {
    ar: {
      ...
      helpTitle: 'بحاجة لمساعدة أو استشارة خاصة؟',
  ...
  // FREE SHIPPING STRATEGY: 20 DH shipping cost, free threshold: 300 DH
  const FREE_SHIPPING_THRESHOLD = 300;
  const isFreeShipping = subtotal >= FREE_SHIPPING_THRESHOLD || qty > 1;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6" style={{ direction: isRtl ? 'rtl' : 'ltr' }}>
      ...
      <div className="grid lg:grid-cols-12 gap-8 items-start relative">
        ...
        <div className="lg:col-span-7 space-y-8">
          ...
          <div id="checkout_form_box" className="bg-white rounded-3xl border border-[#EADFC9]/60 shadow-xl overflow-hidden">
            ...
            <form onSubmit={handleCheckoutSubmit} className="p-6 sm:p-8 space-y-6">
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full text-xs sm:text-sm px-4 py-3.5 pl-10 rounded-xl border border-slate-200 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-[#C5A560] bg-white transition-all font-mono"
              ...
              <button
                type="submit"
                disabled={isOrdering}
                className="w-full inline-flex items-center justify-center gap-2 bg-[#1C352D] hover:bg-[#25443B] disabled:bg-[#1c352d]/80 text-[#FAF6F0] font-black py-4.5 rounded-2xl shadow-xl border border-[#C5A560]/30 hover:scale-[1.01] active:scale-99 transition-all text-xs sm:text-sm cursor-pointer select-none uppercase tracking-wider"
              >
                {isOrdering ? (
          ...
          {/* DYNAMIC BUNDLE OFFERS SYSTEM */}
          <div id="bundle-offers-section" className="space-y-4">
            <div className="space-y-1">
              <h3 className="text-base sm:text-lg font-black text-[#1C352D] flex items-center gap-2">
                <Gift className="w-5 h-5 text-[#C5A560]" />
            ...
            <div className="grid gap-4">
              {sortedBundles.map((b) => {
                const isAdded = !!activeBundles[b.id];
                const isRecommended = b.category === category;
                return (
                  <motion.div
                    key={b.id}
                    whileHover={{ y: -3 }}
                    className={`rounded-3xl border p-5 sm:p-6 transition-all relative overflow-hidden flex flex-col md:flex-row gap-5 ${
                      isAdded 
                        ? 'border-emerald-600 bg-[#EAF0EC]/40 shadow-md' 
                        : isRecommended 
                          ? 'border-[#C5A560] bg-[#FAF6F0]/30 shadow-sm'
                          : 'border-slate-100 bg-white hover:border-[#EADFC9]'
                    }`}
                  >
                    {/* Recommended Tag */}
                    {isRecommended && (
                      <span className="absolute top-0 right-0 bg-[#C5A560] text-white text-[9px] font-black uppercase tracking-wider px-3 py-1 rounded-bl-xl">
                        {isRtl ? 'موصى به لطلبك الحالي ⭐' : 'Recommandé ⭐'}
                      </span>
                    )}

                    {/* Bundle Image */}
                    <div className="w-20 h-20 md:w-24 md:h-24 bg-white rounded-2xl border border-[#EADFC9]/20 p-2 shrink-0 flex items-center justify-center self-center">
                      <img src={b.image} alt={b.name[lang]} className="object-contain max-h-full" referrerPolicy="no-referrer" />
                    </div>

                    {/* Bundle Info */}
                    <div className="space-y-2 flex-1">
                      <div className="flex flex-wrap items-baseline gap-2">
                        <h4 className="text-sm font-black text-[#1C352D] leading-tight">
                          {b.name[lang] || b.name['en']}
                        </h4>
                        <span className="bg-emerald-800 text-white text-[9px] font-black px-2 py-0.5 rounded-full">
                          -{b.savingsPercent}%
                        </span>
                      </div>
                      <p className="text-[10.5px] sm:text-xs text-slate-500 leading-relaxed font-sans font-medium">
                        {b.description[lang] || b.description['en']}
                      </p>

                      {/* Included items breakdown */}
                      <div className="flex flex-wrap gap-1.5 pt-1">
                        {b.products.map(prodId => {
                          const info = getProductInfo(prodId);
                          return (
                            <span key={prodId} className="text-[9px] bg-[#FAF8F5] border border-slate-100 text-slate-600 px-2 py-0.5 rounded-md font-bold font-sans">
                              ✓ {info.name.split(' ')[0]}
                            </span>
                          );
                        })}
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-slate-50">
                        <div className="flex items-center gap-2">
                          <span className="text-slate-400 line-through text-xs font-bold font-sans">
                            {b.regularPrice} DH
                          </span>
                          <span className="text-base font-black text-[#C5A560] font-sans">
                            {b.discountedPrice} DH
                          </span>
                          <span className="text-[9px] text-emerald-800 font-black bg-emerald-50 border border-emerald-100 px-1.5 py-0.5 rounded-md">
                            {isRtl ? 'توصيل مجاني' : 'LIVRAISON OFFERTE'}
                          </span>
                        </div>

                        <button
                          type="button"
                          onClick={() => handleToggleBundle(b.id)}
                          className={`px-4 py-2.5 rounded-xl font-black text-xs uppercase tracking-wider transition-all cursor-pointer select-none active:scale-95 ${
                            isAdded
                              ? 'bg-[#1C352D] text-white hover:bg-[#25443B]'
                              : 'bg-[#FAF6F0] text-[#1C352D] border border-[#E5D5C0]/60 hover:bg-[#C5A560]/10 hover:border-[#C5A560]'
                          }`}
                        >
                          {isAdded ? t.addedToOrder : t.addToOrder}
                        </button>
                      </div>
                    </div>
                  </motion.div>
          ...
          {/* SMART UPSELL SYSTEM (AUTOMATIC STORE PRODUCTS LOADER) */}
          <div className="space-y-4">
            <div className="space-y-1">
              <h3 className="text-base sm:text-lg font-black text-[#1C352D] flex items-center gap-2">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {upsellProducts.map((p) => {
                const addedQty = additionalItems[p.id] || 0;
                ...
                return (
                  <motion.div
                    key={p.id}
                    whileHover={{ y: -3 }}
                    className={`bg-white rounded-3xl border p-4 sm:p-5 transition-all flex flex-col justify-between ${
                      isAdded ? 'border-emerald-600 bg-emerald-50/20 shadow-md' : 'border-slate-100 hover:border-[#EADFC9] shadow-xs'
                    }`}
                  >
                    <div className="space-y-3">
                      {/* Product Image */}
                      <div className="w-full aspect-square bg-[#FAF8F5] rounded-2xl border border-slate-55 p-3 shrink-0 flex items-center justify-center relative">
                        <img src={p.image} alt={p.name} className="object-contain max-h-full max-w-full" referrerPolicy="no-referrer" />
                        {isAdded && (
                          <span className="absolute top-2 right-2 bg-emerald-800 text-white text-[9px] font-black px-2 py-0.5 rounded-full">
                            ✓ {isRtl ? 'مضاف' : 'Ajouté'}
                          </span>
                        )}
                      </div>
                      ...
                      {/* Details */}
                      <div className="space-y-1">
                        <h4 className="text-xs sm:text-sm font-black text-[#1C352D] leading-tight line-clamp-1">
                          {p.name}
                        </h4>
                        ...
                        <div className="space-y-0.5 pt-1">
                          {p.benefits.slice(0, 2).map((benefit, i) => (
                            <span key={i} className="text-[9.5px] text-slate-400 block font-medium leading-tight">
                              ✓ {benefit}
                            </span>
                          ))}
                        </div>
                    ...
                    <div className="pt-3.5 border-t border-slate-50 flex items-center justify-between mt-3.5">
                      <div className="flex items-baseline gap-1" style={{ flexDirection: isRtl ? 'row-reverse' : 'row' }}>
                        <span className="text-base font-black text-[#C5A560] font-sans">{p.price}</span>
                        <span className="text-[9px] font-bold text-[#C5A560]">{isRtl ? 'درهم' : 'DH'}</span>
                      </div>

                      {/* Quantity selector or Add Button */}
                      {isAdded ? (
                        <div className="flex items-center gap-2 bg-slate-50 border border-slate-100 rounded-xl px-1.5 py-0.5">
                          <button
                            type="button"
                            onClick={() => handleUpdateUpsellQty(p.id, addedQty - 1)}
                            className="w-6 h-6 rounded-md hover:bg-white text-slate-600 font-bold flex items-center justify-center text-xs cursor-pointer select-none"
                          >
                            <Minus className="w-3 h-3" />
                          </button>
                          <span className="text-xs font-black text-[#1C352D] font-mono min-w-[12px] text-center select-none">
                            {addedQty}
                          </span>
                          <button
                            type="button"
                        ...
                        <button
                          type="button"
                          onClick={() => handleAddUpsell(p.id, 1)}
                          className="px-3.5 py-2 bg-[#FAF6F0] hover:bg-[#C5A560] hover:text-white border border-[#E5D5C0]/60 text-[#1C352D] rounded-xl font-black text-[10px] uppercase tracking-wider transition-all cursor-pointer select-none"
                        >
                          {isRtl ? 'أضف للطلب' : 'Ajouter'}
                        </button>
                      )}
                    </div>
                  </motion.div>
