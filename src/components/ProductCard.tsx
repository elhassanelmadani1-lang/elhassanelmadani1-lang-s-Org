...
export default function ProductCard({
  ...
  // Find matching wellness packs for this product
  const matchingPacks = wellnessPacks.filter((pack) => {
    return pack.products?.some((p) => {
      if (!prod?.id || !p?.id) return false;
      const norm = (s: string) => s.toLowerCase().replace(/[-_]/g, '');
      return norm(prod.id) === norm(p.id);
    });
  });
            ...
            >
              ...
              <div className="w-full md:w-1/2 p-6 md:p-8.5 flex flex-col justify-between overflow-y-auto max-h-[50vh] md:max-h-none">
                <div>
                  ...
                  {/* Matching Packs Cross-Sell Section */}
                  {matchingPacks.length > 0 && (
                    <div className="mt-6 pt-5 border-t border-neutral-150/60 dark:border-slate-800/60">
                      <h4 className="text-[11px] font-black text-[#C5A560] dark:text-amber-400 uppercase tracking-widest flex items-center gap-1.5 mb-3 select-none">
                        <Sparkles className="w-3.5 h-3.5 animate-pulse" />
                        {lang === 'ar' ? 'متوفر أيضاً في باقات متميزة بخصم إضافي!' : lang === 'fr' ? 'Disponible aussi dans nos Packs !' : 'Also Available in Best-Value Packs!'}
                      </h4>
                      <div className="space-y-2">
                        {matchingPacks.map((pack) => {
                          ...
                          return (
                            <div
                              key={pack.id}
                              className="group/pack flex items-center justify-between p-2.5 rounded-2xl bg-emerald-50/40 dark:bg-emerald-950/10 border border-emerald-100/60 dark:border-emerald-900/20 hover:border-emerald-300/60 dark:hover:border-emerald-800/40 transition-all duration-300 shadow-xs"
                            >
                              <div className="flex items-center gap-3 min-w-0">
                                <div className="w-11 h-11 rounded-lg bg-white dark:bg-slate-900 p-1 border border-neutral-200/50 dark:border-slate-800 flex items-center justify-center shrink-0">
                                  <img
                                    src={pack.image}
                                    alt={packName}
                                    className="w-full h-full object-contain group-hover/pack:scale-105 transition-transform"
                                    referrerPolicy="no-referrer"
                                  />
                                </div>
                                <div className="min-w-0">
                                  <h5 className="text-xs font-black text-[#1F4D3A] dark:text-emerald-400 truncate leading-tight">
                                    {packName}
                                  </h5>
                                  <p className="text-[10px] text-slate-450 dark:text-slate-500 font-bold font-mono mt-0.5">
                                    {lang === 'ar' ? 'وفر حتى' : 'Épargnez'} {pack.originalPrice - pack.price} {t.currency}
                                  </p>
                                </div>
                              </div>
                              <div className="flex items-center gap-2.5 shrink-0">
                                <div className="text-right">
                                  <span className="block text-[10px] text-slate-450 line-through font-mono leading-none">
                                    {pack.originalPrice} {t.currency}
                                  </span>
                                  <span className="block text-[13px] font-black text-emerald-600 dark:text-emerald-400 font-mono leading-none mt-1">
                                ...
                                {onSelectPack && (
                                  <button
                                    ...
                                    className="p-2 rounded-xl bg-white dark:bg-slate-900 border border-neutral-200 dark:border-slate-800 text-slate-500 hover:text-emerald-600 dark:hover:text-emerald-400 hover:border-emerald-200 group-hover/pack:translate-x-0.5 transition-all shadow-xs cursor-pointer flex items-center justify-center"
                                    title={lang === 'ar' ? 'عرض الباقة' : 'Voir le pack'}
                                  >
                                    {isRtl ? (
